...
export class OwnerOrAdminGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        // 1) Route: /houses/:house_id/details
        if (request.params.house_id) {
            houseId = Number(request.params.house_id);
        }

        // 2) Route: /house-images/:id
        else if (request.params.id && request.route.path.includes('house-images')) {
            const image = await this.prisma.houseImage.findUnique({
                where: { id: Number(request.params.id) },
            });
            if (!image) throw new ForbiddenException("Uy rasmi topilmadi");
        ...
        // 3) Route: /house-facilities/:id ← **Yangi qo‘shildi**
        else if (request.params.id && request.route.path.includes('house-facilities')) {
            const facility = await this.prisma.houseFacility.findUnique({
                where: { id: Number(request.params.id) },
            });
            if (!facility) throw new ForbiddenException("Qulaylik topilmadi");
            houseId = facility.house_id;
        }

        // 4) POST vaqtida body dan olish
        else if (request.body.house_id) {
            houseId = Number(request.body.house_id);
        }
